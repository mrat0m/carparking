...
session_start();
$q="select * from booking where status='PAID'";
$records = mysqli_query($conn,"select * from booking where status='PAID'"); // fetch data from database
if(isset($_POST['filter'])){
  extract($_POST);
  if($ord=="pdate"){
    // echo "hiiii";
    $q="select * from booking where status='PAID' order by time desc ";
    $records = mysqli_query($conn,"select * from booking where status='PAID' order by time desc ");
  }
  else if($ord=="pslot"){
    
    $q="select * from booking where status='PAID' order by ps_id asc ";
     $records = mysqli_query($conn,"select * from booking where status='PAID' order by ps_id asc ");
  }
...
if(isset($_POST['srch'])){
  extract($_POST);
  if($jsfilter=="slot"){
    $q="SELECT * FROM booking INNER JOIN `pslot` USING(`ps_id`) WHERE STATUS='PAID' AND `npslot` LIKE '%$search%'";
    $records = mysqli_query($conn,"SELECT * FROM booking INNER JOIN `pslot` USING(`ps_id`) WHERE STATUS='PAID' AND `npslot` LIKE '%$search%'");
  }
  else if($jsfilter=="date"){
    $q="SELECT * FROM booking INNER JOIN `pslot` USING(`ps_id`) WHERE STATUS='PAID' AND `time` LIKE '%$search%'";
    // echo "<script> alert('$search')</script>";
     $records = mysqli_query($conn,"SELECT * FROM booking INNER JOIN `pslot` USING(`ps_id`) WHERE STATUS='PAID' AND `time` LIKE '%$search%'");
...
<html lang="en" class="h-100">
  <head>
    ...
    <meta http-equiv = "refresh" content = "30; url = http://localhost/carparking/admin/bookings.php" />    
<style>
  .main {
  padding: 20px;
  margin-top: 55px;
...
<tr>
  <td align="end" style="padding-right: 5em;">
    <!-- <input type="button" class="btn btn-primary btn-sm" style="width: auto;border-radius: 50px;" value="Download Report" onclick="printDiv()">  -->
    <a href="printsection.php" class="btn btn-primary btn-sm" style=" margin-top: -105px;">Print</a>
  </td>
</tr>
</table>
...
<br><h4 class="txt-title float-md-start mb-0" style="text-align: center;">  <?php if(isset($_POST['filter'])){
         ...
         else{ $_SESSION['head']="All Bookings" ?>Active Bookings <?php }?></h4><br>
	<thead>
