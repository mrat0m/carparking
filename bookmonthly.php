...
<form role="form" action="pgRedirect1.php" method="post">
...
?>
    ...
    <div class="col-lg-3">
      <label class="heading1">No of Months :</label>
        <select class="selectpicker form-control" style="text-align: center;" id="months" name="months" required>
    ...
    <label class="heading1">Slot-rate per day :</label>
    <br>₹ <input name="rate" style="text-align: center;" id="rate1" placeholder="0.00" readonly> </p>
      ...
      If you want to park your vehicle for a hourly <a href="booking.php">click here</a>  
    </div>
        <div class="terms">
    <div class="heading-mini">
      *Parking Slot will only be valid for seleted no of months.You can park your car once payment is complete.
...
$('#months').on('change', function() {
  ...
  rate=$('#rate1').val();
  console.log(rate);
//   var amt= parseFloat((parseInt(months)*30) * parseFloat(rate));
var amt= parseFloat((parseInt(months)*30) * parseFloat(rate));
  console.log(amt);
  $('#amt').val(amt);
}); 
