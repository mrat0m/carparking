...
if(isset($_POST['paynow'])){
    extract($_POST);
   $_SESSION['val']="UPDATE `booking` SET status='PAID',`duration`=`duration`+'$duration',`paid`=`paid`+'$amt' WHERE `bid`='$uid'";
    
    $qwerty=mysqli_fetch_assoc(mysqli_query($conn,"SELECT bid,time FROM booking where bid='$uid'"));
    $_SESSION['bid']=$qwerty['bid'];
    $_SESSION['time0']=$qwerty['time'];
    $_SESSION['ps_id']=$qwerty['ps_id'];
...
<html lang="en" class="h-100">
  ...
  <meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no">
        <title>PARK-iN | Extend Booking</title>
...
<body id="home" onload="myFunction()" style="font-family: 'Times New Roman', Times, serif;" data-spy="scroll"  style="font-family: 'Times New Roman', Times, serif;"  data-spy="scroll" data-target="#navbar-wd" data-offset="98" class=" h-100 text-center text-white bg-dark1">
     <!-- LOADER -->
    <div id="loading"></div>
      <!-- END LOADER -->
  <!-- MENU BAR -->
...
<form action="" method="post">
 <center><div class="col-lg-3">
  ...
  <div class="col-lg-1"></div>
    <div class="col-lg-4">
      <label class="heading1">Parking slot :</label>
      <?php

$slot=mysqli_query($conn,"SELECT * FROM pslot INNER JOIN `booking` USING(`ps_id`) WHERE `bid`='$uid'");
$row = mysqli_fetch_array($slot, MYSQLI_ASSOC);  
?>
<input type="text" name="aps" style="text-align: center;" readonly value="<?php echo $row['npslot'];?>">
    </div>
    <div class="col-lg-2">
    ...
    <br>₹ <input name="rate" style="text-align: center;" placeholder="0.00" id="rate1" readonly value="<?php echo $row['slotrate'];?>"> </p>
    </div>
    ...
    <div class="heading-mini">
      <!-- *Parking Slot is active.   -->
        ...
        <div class="paybtnt">  
            <input type="submit" value="Pay Now" class="btn btn-success btn-lg" name="paynow"><br>
